...
import { PentairAuth } from './auth';
import { PentairApi } from './pentairApi';
import { PentairPumpAccessory } from './accessories/pumpAccessory';
import type { PumpConfig } from './accessories/pumpAccessory';
import { PentairLightAccessory } from './accessories/lightAccessory';
import type { LightConfig } from './accessories/lightAccessory';

/** Shape of the platform configuration block supplied by the user. */
interface PentairPlatformConfig extends PlatformConfig {
  email: string;
  password: string;
  pump?: PumpConfig;
  light?: LightConfig;
 ...
 */
export class PentairHomebridgePlatform implements DynamicPlatformPlugin {
   ...
   */
  public readonly hapApi: API;

  /** Accessories restored from the Homebridge cache on previous runs. */
  private readonly cachedAccessories = new Map<string, PlatformAccessory>();

  private auth!: PentairAuth;
  private pentairApi!: PentairApi;

  constructor(log: Logger, config: PlatformConfig, api: API) {
    this.log = log;
    this.hapApi = api;

    const cfg = config as PentairPlatformConfig;

    if (!cfg.email || !cfg.password) {
      this.log.error(
        `${PLUGIN_NAME}: "email" and "password" are required in the platform config.`,
      );
    ...
    this.hapApi.on('didFinishLaunching', () => {
      ...
      this.initPlatform(cfg).catch((err) => {
        this.log.error(`${PLUGIN_NAME}: fatal error during startup`, err);
      });
    });
  }
   ...
   */
  private async initPlatform(config: PentairPlatformConfig): Promise<void> {
    try {
      ...
      this.log.error(
        'Failed to authenticate with Pentair Cloud. ' +
        'Please check your email and password in the plugin config.',
        err,
      );
    ...
    try {
      ...
      (list as Array<Record<string, unknown>>).forEach((device, i) => {
        this.log.info(
          `  [${i + 1}] name: "${device['name'] ?? device['deviceName'] ?? 'Unknown'}" | ` +
   ...
   */
  private discoverDevices(config: PentairPlatformConfig): void {
    if (config.pump) {
   ...
   */
  private registerPumpAccessory(pumpConfig: PumpConfig): void {
    const uuid = this.hapApi.hap.uuid.generate(
      `${PLUGIN_NAME}-pump-${pumpConfig.deviceId}`,
    );
    ...
    if (accessory) {
      ...
      this.hapApi.registerPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, [
        accessory,
      ]);
    }
    ...
    new PentairPumpAccessory(this, accessory, this.pentairApi, pumpConfig);
   ...
   */
  private registerLightAccessory(lightConfig: LightConfig): void {
    const uuid = this.hapApi.hap.uuid.generate(
      `${PLUGIN_NAME}-light-${lightConfig.deviceId}`,
    );
    ...
    new PentairLightAccessory(this, accessory, this.pentairApi, lightConfig);
   ...
   */
  private pruneStaleAccessories(config: PentairPlatformConfig): void {
    // Determine which UUIDs are expected.
    const expectedUuids = new Set<string>();
    if (config.pump) {
      expectedUuids.add(
        this.hapApi.hap.uuid.generate(
          `${PLUGIN_NAME}-pump-${config.pump.deviceId}`,
        ),
      );
    }
    if (config.light) {
      expectedUuids.add(
        this.hapApi.hap.uuid.generate(
          `${PLUGIN_NAME}-light-${config.light.deviceId}`,
        ),
      );
    }
    ...
    for (const [uuid, accessory] of this.cachedAccessories) {
      if (!expectedUuids.has(uuid)) {
        this.log.info(
          `Removing stale accessory from cache: ${accessory.displayName}`,
        );
    ...
    if (stale.length > 0) {
      this.hapApi.unregisterPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, stale);
    }
  }
}
